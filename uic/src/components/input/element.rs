use gpui::{
    ...
    PaintQuad, Pixels, ShapedLine, Style, TextRun, UnderlineStyle, Window, fill, point, prelude::*,
    px, relative, size,
};
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let indicator_top = bounds.top() + (bounds.size.height - appearance.caret_height) / 2.;
        let indicator_bottom = indicator_top + appearance.caret_height;
        let (selection, cursor) = if disabled {
            ...
            (
                ...
                Some(fill(
                    Bounds::new(
                        point(bounds.left() + cursor_pos, indicator_top),
