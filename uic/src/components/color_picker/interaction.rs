use std::{cell::Cell, rc::Rc};

use gpui::{
    App, Bounds, DispatchPhase, Element, ElementId, FocusHandle, GlobalElementId, Hitbox,
    HitboxBehavior, HitboxId, InspectorElementId, IntoElement, MouseButton, MouseDownEvent,
    MouseMoveEvent, MouseUpEvent, Pixels, Point, Refineable as _, Style, StyleRefinement, Styled,
    Window,
};

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum InteractionPhase {
    Preview,
    Commit,
}

#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub(crate) struct NormalizedPoint {
    pub x: f32,
    pub y: f32,
}

type Callback = Rc<dyn Fn(NormalizedPoint, InteractionPhase, &mut Window, &mut App)>;

#[derive(Clone, Default)]
pub(crate) struct CaptureToken(Rc<Cell<Option<HitboxId>>>);

pub(crate) struct ColorInteraction {
    capture_token: CaptureToken,
    focus_handle: FocusHandle,
    callback: Callback,
    style: StyleRefinement,
}

impl ColorInteraction {
    pub fn new(
        capture_token: CaptureToken,
        focus_handle: FocusHandle,
        callback: impl Fn(NormalizedPoint, InteractionPhase, &mut Window, &mut App) + 'static,
    ) -> Self {
        Self {
            capture_token,
            focus_handle,
            callback: Rc::new(callback),
            style: StyleRefinement::default(),
        }
    }
}

impl IntoElement for ColorInteraction {
    type Element = Self;

    fn into_element(self) -> Self::Element {
        self
    }
}

impl Element for ColorInteraction {
    type RequestLayoutState = Style;
    type PrepaintState = Hitbox;

    fn id(&self) -> Option<ElementId> {
        None
    }

    fn source_location(&self) -> Option<&'static core::panic::Location<'static>> {
        None
    }

    fn request_layout(
        &mut self,
        _id: Option<&GlobalElementId>,
        _inspector_id: Option<&InspectorElementId>,
        window: &mut Window,
        cx: &mut App,
    ) -> (gpui::LayoutId, Self::RequestLayoutState) {
        let mut style = Style::default();
        style.refine(&self.style);
        let layout_id = window.request_layout(style.clone(), [], cx);
        (layout_id, style)
    }

    fn prepaint(
        &mut self,
        _id: Option<&GlobalElementId>,
        _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>,
        _request_layout: &mut Style,
        window: &mut Window,
        _cx: &mut App,
    ) -> Self::PrepaintState {
        let hitbox = window.insert_hitbox(bounds, HitboxBehavior::Normal);
        if let Some(previous_hitbox) = self.capture_token.0.get()
            && window.captured_hitbox() == Some(previous_hitbox)
        {
            window.capture_pointer(hitbox.id);
        }
        self.capture_token.0.set(Some(hitbox.id));
        hitbox
    }

    fn paint(
        &mut self,
        _id: Option<&GlobalElementId>,
        _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>,
        style: &mut Style,
        hitbox: &mut Self::PrepaintState,
        window: &mut Window,
        cx: &mut App,
    ) {
        let down_hitbox = hitbox.clone();
        let down_focus = self.focus_handle.clone();
        let down_callback = self.callback.clone();
        let move_hitbox = hitbox.clone();
        let move_callback = self.callback.clone();
        let up_hitbox = hitbox.clone();
        let up_callback = self.callback.clone();

        style.paint(bounds, window, cx, move |window, _| {
            window.on_mouse_event(move |event: &MouseDownEvent, phase, window, cx| {
                if phase == DispatchPhase::Bubble
                    && event.button == MouseButton::Left
                    && down_hitbox.is_hovered(window)
                {
                    window.capture_pointer(down_hitbox.id);
                    down_focus.focus(window, cx);
                    down_callback(
                        normalize(event.position, down_hitbox.bounds),
                        InteractionPhase::Preview,
                        window,
                        cx,
                    );
                    cx.stop_propagation();
                }
            });
            window.on_mouse_event(move |event: &MouseMoveEvent, phase, window, cx| {
                if phase == DispatchPhase::Capture
                    && event.dragging()
                    && window.captured_hitbox() == Some(move_hitbox.id)
                {
                    move_callback(
                        normalize(event.position, move_hitbox.bounds),
                        InteractionPhase::Preview,
                        window,
                        cx,
                    );
                    cx.stop_propagation();
                }
            });
            window.on_mouse_event(move |event: &MouseUpEvent, phase, window, cx| {
                if phase == DispatchPhase::Capture
                    && event.button == MouseButton::Left
                    && window.captured_hitbox() == Some(up_hitbox.id)
                {
                    up_callback(
                        normalize(event.position, up_hitbox.bounds),
                        InteractionPhase::Commit,
                        window,
                        cx,
                    );
                    cx.stop_propagation();
                }
            });
        });
    }
}

impl Styled for ColorInteraction {
    fn style(&mut self) -> &mut StyleRefinement {
        &mut self.style
    }
}

fn normalize(position: Point<Pixels>, bounds: Bounds<Pixels>) -> NormalizedPoint {
    let width = f32::from(bounds.size.width).max(1.0);
    let height = f32::from(bounds.size.height).max(1.0);
    NormalizedPoint {
        x: (f32::from(position.x - bounds.origin.x) / width).clamp(0.0, 1.0),
        y: (f32::from(position.y - bounds.origin.y) / height).clamp(0.0, 1.0),
    }
}
